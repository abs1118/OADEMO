...
addFile({
name: entry.name, type: 'directory', path: (path ? path : '') + entry.name
});
...
/**
 * ! AngularJS file upload/drop directive with progress and abort FileAPI Flash
 * shim for old browsers not supporting FormData
 * 
 * @author Danial <danial.farid@gmail.com>
 * @version 2.0.5
 */
...
type: 'load', loaded: xhr.__loaded, total: xhr.__total, target: xhr, lengthComputable: true
});
if (!err && xhr.__listeners['loadend'])
xhr.__listeners['loadend']({
type: 'loadend', loaded: xhr.__loaded, total: xhr.__total, target: xhr, lengthComputable: true
});
if (err === 'abort' && xhr.__listeners['abort'])
xhr.__listeners['abort']({
type: 'abort', loaded: xhr.__loaded, total: xhr.__total, target: xhr, lengthComputable: true
});
...
// fix flash issue that doesn't call complete if there is no response text from
// the server
var _this = this
setTimeout(function() {
...
var e = {
type: type, target: _this, loaded: evt.loaded, total: evt.total, error: evt.error
};
